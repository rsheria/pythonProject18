...
class FileProcessor:
    ...
    def process_downloads(
        ...
        try:
            ...
            if self._detect_if_single_item(moved_files):
                ...
                processed_files = self._process_as_single_item(moved_files, thread_dir, cleaned_thread_title)
            else:
                # Multiple distinct files => process each with its own original name
                processed_files = self._process_multi_distinct(moved_files, thread_dir)
        ...
        for file_path in moved_files:
            if self._is_archive_file(file_path):
                if file_path in self.processed_files:
                    ...
                    continue
                new_archives = self.handle_archive_file(file_path, thread_dir, thread_title)
                if new_archives:
    ...
    def _process_multi_distinct(
        self, moved_files: List[Path], thread_dir: Path
    ) -> List[str]:
        """
    ...
    def handle_archive_file(
        ...
        try:
            ...
            for attempt in range(max_retries):
                logging.info(f"Extracting {original_format} archive: {archive_path} (Attempt {attempt + 1})")
                extract_success = self._extract_archive(archive_path, extract_dir)
                if extract_success:
