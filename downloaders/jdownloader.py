...
class JDownloaderDownloader(BaseDownloader):
    ...
    def _extract_host_name(self, url: str) -> str:
        ...
        try:
            if 'rapidgator' in url.lower():
                return "Rapidgator"
            elif 'katfile' in url.lower():
                return "Katfile"
            elif 'mega.nz' in url.lower():
                return "Mega"
            elif 'nitroflare' in url.lower():
                return "Nitroflare"
            elif 'ddownload' in url.lower():
                return "DDownload"
            elif 'xup.in' in url.lower():
                return "Xup.in"
            elif 'f2h.io' in url.lower():
                return "F2H.io"
            elif 'filepv.com' in url.lower():
                return "FilePV"
            elif 'filespayouts.com' in url.lower():
                return "FilesPayouts"
            elif 'uploady.io' in url.lower():
                return "Uploady"
            elif 'keeplinks.org' in url.lower():
                return "Keeplinks"
            else:
                return "JDownloader"
        except Exception:
