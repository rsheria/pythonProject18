...
def get_downloader_for(url: str, bot):
    ...
    supported_hosts = [
        'rapidgator.net', 'nitroflare.com', 'ddownload.com', 'katfile.com', 'mega.nz',
        'xup.in', 'f2h.io', 'filepv.com', 'filespayouts.com', 'uploady.io',  # New hosts
    ...
    if jd_available:
        ...
        elif any(host in u for host in ['xup.in', 'f2h.io', 'filepv.com', 'filespayouts.com', 'uploady.io']):
...
class DownloadWorker(QThread):
    ...
    def __init__(self, bot, file_processor, selected_rows, gui):
        ...
        logging.info(f"🆕 DownloadWorker created with session ID: {self.worker_session_id}")

        logging.debug("DownloadWorker initialized, max_concurrent=%d", self.max_concurrent)
    ...
    def get_download_hosts_priority(self):
        ...
        try:
            ...
            if not isinstance(priority, list) or not priority:
                logging.warning("⚠️ Invalid download_hosts_priority in settings, using defaults")
            ...
            logging.info(f"📋 Using custom download hosts priority: {priority[:3]}{'...' if len(priority) > 3 else ''}")
            return priority
        except Exception as e:
            logging.error(f"❌ Error reading download_hosts_priority: {e}, using defaults")
            return list(default_priority) + [h for h in extra_hosts if h not in default_priority]
    ...
    def run(self):
        try:
            ...
            while not self.is_cancelled:
                ...
                while (len(self.active_link_downloads) < self.max_concurrent
                ...
                with self.lock:
                    finished = [lid for lid,info in self.active_link_downloads.items()
                    ...
                    for lid in finished:
                        try:
                            ...
                            if (self.thread_info_map[tid]['done_count']
    ...
    def initialize_download_queue(self):
        ...
        for row in self.selected_rows:
            ...
            logging.debug("Row %d => title=%r, category=%r, id=%r",
                          row, thread_title, category_name, thread_id)
            ...
            for priority_host in priority_hosts:
                if priority_host in organized_links:
                    ...
                    logging.info(f"📥 Selected {priority_host} for '{thread_title}' (Priority #{priority_hosts.index(priority_host)+1})")
                    break
    ...
    def start_link_download(self, info):
        ...
        logging.debug(f"📶 Emitting file_created signal (Session: {self.worker_session_id}, File: {filename})")
        ...
        def download_job():
            ...
            def progress_cb(cur, tot, fn, *args):
                ...
                try:
                    ...
                    if hasattr(self, 'file_progress_update'):
                        logging.debug(f"📊 Emitting progress update (Session: {self.worker_session_id}, File: {display_name}, Progress: {pct}%)")
                        self.file_progress_update.emit(
                            link_id, pct, "Downloading", cur, tot, display_name, speed, eta
                        )
            ...
            if files_after_scan > files_before_scan:
                logging.info(f"📁 Found {files_after_scan - files_before_scan} additional files in thread directory")
            elif files_before_scan > 0:
                logging.info(f"📁 Using {files_before_scan} files from downloader (JDownloader)")
            ...
            try:
                if hasattr(self, 'file_progress_update') and info['new_files']:
                    filename = os.path.basename(info['new_files'][-1]) if info['new_files'] else "Unknown"
                    self.file_progress_update.emit(
    ...
    def process_thread_files(self, thread_id):
        ...
        try:
            ...
            password = self.gui.process_threads.get(info['category_name'], {}).get(
                info['thread_title'], {}
            ).get('password')
            ...
            if processed:
                ...
                entry = self.gui.process_threads[
                    info['category_name']
                ][info['thread_title']]
                entry.update({
                    'file_name': os.path.basename(main),
                    'file_path': str(main)
                })
                self.gui.save_process_threads_data()
                logging.info(
                    "Processed main file for '%s': %s",
                    info['thread_title'], main
                )
