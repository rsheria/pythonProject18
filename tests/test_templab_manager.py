...
def test_convert_autolinks_without_placeholder(monkeypatch):
    def fake_apply_template(bb, cat, auth, thread_title=""):
        return "BODY"
    ...
    def fake_get_user_manager():
        return DummyUserMgr()

    def fake_apply_links_template(tpl, links):
        return "BLOCK"

    monkeypatch.setattr(templab_manager, "apply_template", fake_apply_template)
    monkeypatch.setattr(
        templab_manager, "_load_cfg", lambda c, a: {"template": "{TITLE}"}
    )
    monkeypatch.setattr(templab_manager, "load_global_prompt", lambda: "")
    monkeypatch.setattr(templab_manager, "parse_bbcode_ai", lambda bb, pr: {"title": "T"})
    import utils
    monkeypatch.setattr(utils, "apply_links_template", fake_apply_links_template)
    import core.user_manager as user_manager

    monkeypatch.setattr(user_manager, "get_user_manager", fake_get_user_manager)

    thread = {
        "category": "c",
        "author": "a",
        "bbcode_original": "X",
        "links": {"audio": {"ddownload": ["u"]}},
    }
    result = templab_manager.convert(thread, apply_hooks=False)
